(function readyJS(win, doc) {
    ...
    const input = doc.getElementById('amount');
    let loremText = doc.querySelector('.lorem-text')
    const errorText = doc.querySelector('.error-text')
    let inputValue = parseInt(input.value);

    //CHECK IF THE VALUE IS 1-9
    function checkValue() {
        inputValue = parseInt(input.value); //GETS A THE CURRENT INPUT VALUE FOR THE INPUTVALUE VAR
        if (isNaN(inputValue) || inputValue <= 0 || inputValue >= 10) {
            errorText.textContent = "Please enter a valid value (1 - 9)";
            return false;
        } else {
    ...
    //CHECK THE VALUE WHEN THE INPUT VALUE IS CHANGED
    ...
    form.addEventListener('submit', function (e) {
        e.preventDefault();
        if (checkValue()) {
            let tempText = textArr.slice(0, inputValue); //O SLICE ESTÁ PEGANDO UM PEDAÇO DO ARRAY, COMEÇANDO DO INDEX 0
            tempText = tempText.map(function (paragraph) { //PARA MANTER O MAP, É NECESSÁRIO USAR UMA ATRIBUIÇÃO A UMA VARIÁVEL
